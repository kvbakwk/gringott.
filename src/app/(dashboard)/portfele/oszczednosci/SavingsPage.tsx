...
export default function SavingsPage() {
  ...
  const [activeForm, setActiveForm] = useState<"deposit" | "withdrawal" | null>(
    null,
  );
  ...
  const savingsWallets = useMemo(
    () => wallets.filter((w) => w.wallet_type_id === 3),
    [wallets],
  );
  const savingsWalletIds = useMemo(
    () => new Set(savingsWallets.map((w) => w.id)),
    [savingsWallets],
  );

  const totalBalance = useMemo(
    () => savingsWallets.reduce((a, b) => a + b.balance, 0),
    [savingsWallets],
  );
  ...
  const savingsItems = useMemo(() => {
    ...
    const trs = transfers
      .filter(
        (t) =>
          savingsWalletIds.has(t.to_wallet_id) ||
          savingsWalletIds.has(t.from_wallet_id),
      )
      .map((t) => {
  ...
  const currentMonthGrowth = useMemo(() => {
    ...
    return thisMonth.reduce(
      (sum, t) => (t.income ? sum + t.amount : sum - t.amount),
      0,
    );
  }, [savingsItems]);
      ...
      <div className="flex justify-between items-center w-full mb-4">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-on-surface-variant max-w-2xl">
            twoje centrum finansowego bezpieczeństwa. zarządzaj swoimi
            oszczędnościami w jednym miejscu.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="p-8 bg-surface rounded-3xl flex justify-between items-center w-full">
          <div className="flex flex-col">
            <span className="text-xs font-bold text-on-surface-variant tracking-wider uppercase mb-1">
              suma oszczędności
            </span>
            <div
              className={`text-4xl font-bold flex items-baseline gap-2 ${totalBalance > 0 ? "text-green-600" : "text-on-surface"}`}
            >
              <Value amount={totalBalance} show={isReady} suffix="" />
              <span
                className={`text-xl font-medium ${totalBalance > 0 ? "text-green-600/70" : "text-on-surface/50"}`}
              >
                PLN
              </span>
            </div>
            {currentMonthGrowth !== 0 && (
              <div
                className={`mt-3 px-3 py-1 rounded-md text-sm font-medium w-fit flex items-center gap-1 ${currentMonthGrowth >= 0 ? "bg-green-100/50 text-green-800" : "bg-red-100/50 text-red-800"}`}
              >
                <Icon className="text-sm">
                  {currentMonthGrowth >= 0 ? "trending_up" : "trending_down"}
                </Icon>
                <span>
                  {currentMonthGrowth >= 0 ? "+" : ""}
                  {parseMoney(currentMonthGrowth)} PLN w tym miesiącu
                </span>
              </div>
            )}
          </div>
              ...
              savingsItems.map((item, idx) => (
                ...
                >
                  <div className="flex items-center gap-6">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 ${item.income ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                    >
                      <Icon className="text-2xl">
                        {item.income ? "south_west" : "north_east"}
                      </Icon>
                    </div>
                  ...
                  <div className="text-right">
                    <span
                      className={`text-lg font-bold ${item.income ? "text-green-600" : "text-red-600"}`}
                    >
                      {item.income ? "+" : "-"}
                      {parseMoney(item.amount)}{" "}
                      <span className="text-xs opacity-60">PLN</span>
                    </span>
                  </div>
                </div>
              ))
