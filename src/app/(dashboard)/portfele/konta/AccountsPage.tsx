          ...
          {isReady && bankAccounts.length === 0 && (
            <div className="col-span-full flex flex-col items-center justify-center p-12 bg-white/50 opacity-50 rounded-3xl border-2 border-dashed border-outline-variant/30 gap-2">
              <div className="w-12 h-12 rounded-full bg-surface-variant/50 flex items-center justify-center text-on-surface-variant mb-1">
                <Icon className="text-2xl text-on-surface-variant">
                  account_balance
                </Icon>
              </div>
              ...
              <p className="text-xs text-on-surface-variant max-w-sm text-center opacity-70">
                nie dodałeś jeszcze żadnego konta bankowego. kliknij &quot;dodaj
                konto&quot; powyżej, aby zacząć śledzić swoje środki.
              </p>
            </div>
          )}
        </div>
...
function AccountCard({
  ...
  const isEuro =
    wallet.name.toLowerCase().includes("eur") ||
    wallet.name.toLowerCase().includes("revolut");
  ...
  return (
    <div className="flex items-center justify-between p-5 bg-surface rounded-2xl transition-all group relative">
      ...
      <div className="flex items-center gap-4">
        <div
          className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${type === "bank" ? "bg-blue-900 text-white" : "bg-surface-variant text-on-surface"}`}
        >
          {type === "bank" ? (
        ...
        <div className="flex flex-col">
          <h3 className="font-bold text-on-surface leading-tight text-lg">
            {wallet.name}
          </h3>
        </div>
      </div>
