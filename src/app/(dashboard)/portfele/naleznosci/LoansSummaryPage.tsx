...
export default function LoansSummaryPage() {
    ...
    return loans
      .map((loan) => {
        const loanTransactions = transactions.filter(
          (t) => Number(t.loan_id) === Number(loan.id),
        );
        const paidAmount = loanTransactions.reduce(
          (sum, t) => sum + t.amount,
          0,
        );
        const isPaid =
          paidAmount >= loan.total_amount || loan.status === "paid";
  ...
  const debts = useMemo(
    () => processedLoans.filter((l) => !l.is_given),
    [processedLoans],
  );
  ...
  const totalDebts = useMemo(
    () =>
      debts
        .filter((l) => !l.isPaid)
        .reduce(
          (acc, curr) => acc + (curr.total_amount - curr.computedPaidAmount),
          0,
        ),
    [debts],
  );
  ...
  useEffect(() => {
    if (["new", "repay", "edit", "delete"].includes(operation)) {
      ...
      if (focusEl) {
        ...
        if (operation === "new")
          focusEl.classList.add("border-primary", "ring-4", "ring-primary/20");
        else if (operation === "edit")
          focusEl.classList.add("border-yellow-500");
        else if (operation === "repay")
          focusEl.classList.add("border-blue-700");
      ...
      <div className="flex justify-between items-start w-full">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-on-surface-variant text-sm">
            zarządzaj swoimi długami i wierzytelnościami
          </p>
        </div>
        ...
        <div className="flex flex-col gap-6 bg-surface rounded-3xl p-6">
          ...
          <div className="flex flex-col gap-4">
            {isLoading ? (
              ...
              <div className="text-center py-8 text-on-surface-variant opacity-60 italic">
                Brak długów
              </div>
            )}
          </div>
          ...
          <div className="flex justify-center items-center gap-[30px] w-[500px] h-fit py-[40px] bg-surface border-1 border-error rounded-2xl shadow-lg transition-all">
            <div className="flex flex-col justify-center items-center gap-[50px] w-[400px] px-[10px] py-[10px]">
              ...
              <div className="flex justify-end items-center gap-[10px] w-full">
                <OutlinedButton onClick={closeForm}>nie, anuluj</OutlinedButton>
                <FilledButton className="error" onClick={handleDeleteLoan}>
                  tak, usuń
                </FilledButton>
              </div>
            </div>
...
function LoanCard({
  ...
  const percentage = isPaid
    ...
    : Math.min(
        100,
        Math.round((loan.computedPaidAmount / loan.total_amount) * 100),
      );
  ...
  const lastPaymentDate =
    sortedTransactions.length > 0
      ? parseDate(new Date(sortedTransactions[0].date))
      : isPaid
        ? parseDate(new Date(loan.updated_at))
        : "Brak";

  // Colors based on type
  const amountColor = isDebt ? "text-red-700" : "text-green-700";
  const initialsBg = isDebt
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${initialsBg}`}
          >
            {subject
              ? subject.name
                  .split(" ")
        ...
        <div className="text-right">
          <div
            className={`text-lg font-bold ${isPaid ? "text-on-surface-variant" : amountColor}`}
          >
            {isPaid ? "0,00 PLN" : `${parseMoney(remaining)} PLN`}
          </div>
          {isPaid ? (
            <div className="text-xs text-on-surface-variant">
              Spłacono: {parseMoney(loan.total_amount)} PLN
            </div>
          ) : (
            <div className="text-xs text-on-surface-variant">
              z {parseMoney(loan.total_amount)} PLN
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1 mb-4">
        ...
        <div
          className={`w-full h-1.5 rounded-full overflow-hidden ${isPaid ? "bg-surface-variant" : progressTrack}`}
        >
          <div
