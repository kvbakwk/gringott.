...
export default function HomePage() {
  ...
  useEffect(() => {
    if (isReady) {
      ...
      const investments = wallets
        .filter((wallet) =>
          [7, 8, 9, 10, 11, 12, 13, 14].includes(wallet.wallet_type_id),
        )
        .reduce((a, b) => a + b.balance, 0);
  ...
  return (
    <div className="flex-1 grid grid-rows-[1fr_auto] h-full p-8 gap-8 overflow-y-auto">
      {/* Header */}
      <div className="flex justify-between items-center w-full pl-6">
        <div>
          ...
          <p className="text-on-surface-variant text-sm mt-1">
            oto podsumowanie Twoich finansów na dziś
          </p>
        </div>
...
function BreakdownCard({
  ...
  return (
    <div
      className={`p-6 bg-surface rounded-3xl flex justify-between gap-4 min-h-[140px] border-2 border-transparent ${borderColor} transition-[border-color] cursor-pointer`}
      onClick={onClick}
    >
      <div className="flex flex-col justify-between items-start">
        <div
          className={`w-[36px] h-[36px] rounded-2xl bg-surface-variant flex items-center justify-start ${textColor}`}
        >
          <Icon className="w-[36px] h-[36px] text-3xl">{icon}</Icon>
        </div>
        <div className="text-xl text-on-surface-variant font-medium pt-10">
          {title}
        </div>
      </div>
      <div className="flex flex-col justify-between items-end text-right">
        ...
        {amount > 0 && (
          <div
            className={`flex items-center text-base font-bold gap-1 mt-4 ${isUp ? "text-green-600" : "text-red-600"}`}
          >
  ...
  return (
    <div className="p-6 bg-surface rounded-3xl h-full flex flex-col justify-center">
      <div className="flex items-center gap-3 mb-2">
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center ${isIncome ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
        >
      ...
      <div className="flex items-center gap-2">
        {show && diff !== 0 ? (
          <>
            <div
              className={`flex items-center px-2 py-0.5 rounded-lg text-xs font-bold gap-1 ${bgClass} ${colorClass}`}
            >
  ...
  const data = useMemo(() => {
    ...
    return months.map((monthStart, idx) => {
      ...
      const amount = transactions
        .filter(
          (t) => !t.income && t.date >= monthStart && t.date < nextMonthStart,
        )
        .reduce((a, b) => a + b.amount, 0);
          ...
          return (
            ...
            >
              <div className="w-full relative flex items-end justify-center h-full">
                ...
                >
                  <div
                    className={`absolute bottom-0 w-full h-full ${item.isCurrent ? "bg-[#636c5b]" : "bg-[#747c6a]"} opacity-90 group-hover:opacity-100 transition-opacity`}
                  />
                </div>
                <div className="absolute -top-8 text-xs font-bold text-on-surface bg-surface shadow-sm px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none whitespace-nowrap">
                  {parseMoney(item.amount)} zł
                </div>
              </div>
              <div
                className={`text-xs font-medium ${item.isCurrent ? "text-on-surface font-bold" : "text-on-surface-variant"}`}
              >
                {item.label}
              </div>
            </div>
