...
export default function LoansHistoryPage() {
  ...
  const getSelectedEvent = () =>
    historyEvents.find((e) => e.id === selectedEventId);
    ...
    <div className="relative grid grid-rows-[50px_1fr] w-full h-full">
      ...
      <div className="flex flex-col w-full h-full overflow-x-auto">
        <div className="flex flex-col min-w-[1510px] h-full">
          ...
          <div
            className={`flex w-full flex-1 min-h-0 px-[20px] pb-[20px] overflow-y-auto scroll-none ${!isLoading ? "flex-col" : "justify-center items-center"}`}
          >
            {isLoading ? (
...
function HistoryRow({
  ...
  const subject = subjects.find(
    (s) => s.id === (isLoan ? data.subject_id : data.subject_id),
  );
  ...
  const description = isLoan
    ? data.name || "pożyczka"
    : data.description || "spłata";
  const typeLabel = isLoan
    ? data.is_given
      ? "udzielenie"
      : "zaciągnięcie"
    : "spłata";
  // For loans, wallet/method are not directly applicable in the same way, or we could show "-"
  const walletName = !isLoan
    ? (wallets.find((w) => w.id === data.wallet_id)?.name ?? "gotówka")
    : "-";
  ...
  return (
    ...
    >
      <div className="w-[20px] h-full flex items-center justify-center">
        <Icon
          className={
            isLoan ? "text-primary text-[20px]" : "text-green-600 text-[20px]"
          }
        >
      ...
      <div
        className={`flex justify-center items-center font-semibold text-xl w-[160px] ${isLoan ? "text-primary" : "text-green-800"}`}
      >
      ...
      >
        ...
        <IconButton
          ...
          onClick={() => onDelete(rowEl.current!)}
        >
          <Icon>delete</Icon>
        </IconButton>
      </div>
