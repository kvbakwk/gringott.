...
export async function getLoansByUserId(userId: number, since?: Date): Promise<LoanT[]> {
    try {
        const query = since
...
export async function updateLoan(userId: number, loanId: number, data: Partial<LoanT>): Promise<LoanT | null> {
    try {
...
export async function repayLoan(
    ...
    try {
        ...
        const { rows: methodRows } = await client.query(`SELECT id FROM public.methods WHERE deleted_at IS NULL LIMIT 1`);
