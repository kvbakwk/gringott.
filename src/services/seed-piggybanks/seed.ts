...
export async function seedPiggybanksAction() {
  try {
    ...
    for (const w of walletsToSeed) {
      const existing = await pool.query('SELECT id FROM public.wallets WHERE name = $1 AND user_id = 1 AND wallet_type_id = 5 AND deleted_at IS NULL', [w.name]);

      if (existing.rowCount === 0) {
        await pool.query('INSERT INTO public.wallets (name, balance, user_id, wallet_type_id) VALUES ($1, $2, 1, 5)', [w.name, w.balance]);
      }
    }
    ...
    for (const item of piggybankData) {
      const walletRes = await pool.query('SELECT id FROM public.wallets WHERE name = $1 AND user_id = 1 AND wallet_type_id = 5 AND deleted_at IS NULL', [item.name]);

      if (walletRes.rowCount > 0) {
        const walletId = walletRes.rows[0].id;
        const txExisting = await pool.query('SELECT id FROM public.transactions WHERE wallet_id = $1 AND description = $2 AND amount = $3 AND deleted_at IS NULL', [walletId, item.desc, item.amount]);
