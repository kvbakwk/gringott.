...
export default function LoanForm({
  ...
  const [subjectId, setSubjectId] = useState<number | null>(
    initialData ? initialData.subject_id : null,
  );
  const [name, setName] = useState<string>(initialData?.name || "");
  const [amount, setAmount] = useState<string>(
    initialData ? initialData.total_amount.toString() : "",
  );
  const [isGiven, setIsGiven] = useState<string>(
    initialData ? (initialData.is_given ? "given" : "received") : "given",
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isSubjectValid = subjectId
