...
export default function NewGoalForm({ 
  ...
  const handleSubmit = async (
    ...
    const balance = parseFloat(
      (formData.get("balance") as string).replace(/\s/g, "").replace(",", ".")
    );
    const targetAmount = parseFloat(
      (formData.get("targetAmount") as string || "0").replace(/\s/g, "").replace(",", ".")
    );

    const isNameValid = validateWalletName(name);
    const isBalanceValid = validateWalletBalance(balance);
    const isTargetValid = targetAmount > 0;

    if (isNameValid && isBalanceValid && isTargetValid) {
      setPending(true);
      createWalletAPI(name, balance, userId, 4, selectedIcon, targetAmount)
        .then((res) => {
          if (res?.createWallet) {
            resetForm();
            successOperation();
            ...
            setBalanceErr(res?.balanceErr || false);
          }
        })
        .catch(() => setError(true))
