...
import { AssetT } from "@/types/asset";
import { WalletTypeT } from "@/types/wallet";
...
export default function AssetsTable({
  ...
  const filteredAssets = useMemo(() => {
    if (filter === "profitable")
      return processedAssets.filter((a) => a.isPositive);
    if (filter === "losing")
      return processedAssets.filter((a) => !a.isPositive);
  ...
  return (
    <div className="bg-surface rounded-3xl flex flex-col h-full relative text-on-surface overflow-hidden">
      ...
      <div
        className={`flex-shrink-0 bg-surface z-40 transition-all duration-300 rounded-t-3xl ${
          scrolled
            ? "shadow-sm border-b border-outline-variant/60"
            : "border-b border-white/0"
        }`}
      >
        {/* Title and FiltersRow */}
        <div className="px-8 pt-6 pb-4 flex justify-between items-center">
          <h2 className="text-xl font-bold text-on-surface lowercase">
            twoje aktywa
          </h2>
      ...
      >
        <div className="flex flex-col gap-1">
          {filteredAssets.map((asset) => {
            const typeBadge =
              ASSET_TYPE_STYLES[asset.type] || ASSET_TYPE_STYLES.other;
              ...
              >
                <div className="col-span-4 flex items-center gap-4">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="font-bold text-on-surface leading-tight truncate">
                      {asset.name}
                    </span>
                ...
                <div className="col-span-2 flex justify-center">
                  <span
                    className={`px-2.5 py-1 rounded-lg text-[10px] font-black tracking-wider ${typeBadge}`}
                  >
                ...
                <div className="col-span-2 text-center flex flex-col justify-center">
                  <span className="text-sm font-bold text-on-surface">
                    {parseMoney(asset.current_price || 0)}{" "}
                ...
                <div
                  className={`col-span-2 text-right text-sm font-black flex flex-col items-end justify-center ${asset.isPositive ? "text-green-600" : "text-error"}`}
                >
                  <div className="flex items-center gap-1">
                    {asset.isPositive ? "+" : ""}
                    {parseMoney(asset.profitLoss)} {asset.currency}
                    <Icon className="text-sm font-black">
                      {asset.isPositive ? "trending_up" : "trending_down"}
                    </Icon>
                  </div>
        ...
        {filteredAssets.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-on-surface-variant/40">
            ...
            <p className="text-sm font-bold tracking-tight lowercase">
              brak aktywów spełniających kryteria.
            </p>
          </div>
        )}
      </div>
