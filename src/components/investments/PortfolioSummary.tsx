...
export default function PortfolioSummary({
  ...
  const totalInvestedValue = assets.reduce(
    (acc, curr) => acc + curr.quantity * (curr.current_price || 0),
    0,
  );
  const totalInvestedCost = assets.reduce(
    (acc, curr) => acc + curr.quantity * (curr.avg_buy_price || 0),
    0,
  ); // Cost basis for calculating profit on assets
  ...
  const profitLossPercentage =
    totalInvestedCost > 0 ? (profitLoss / totalInvestedCost) * 100 : 0;
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 w-full mb-2">
      ...
      <div className="bg-surface p-6 rounded-3xl flex flex-col justify-between">
        ...
        <div className="text-[10px] text-on-surface-variant opacity-60">
          gotówka: {parseMoney(totalCash)} / aktywa:{" "}
          {parseMoney(totalInvestedValue)}
        </div>
      </div>
      ...
      <div className="bg-surface p-6 rounded-3xl flex flex-col justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-2xl font-bold ${statusColorClass}`}>
            {isReady
              ? `${profitLoss > 0 ? "+" : ""}${parseMoney(profitLoss)} PLN`
              : "..."}
          </span>
          {isReady && totalInvestedCost > 0 && (
            <span
              className={`${statusBgClass} ${statusColorClass} text-xs font-bold px-2 py-1 rounded-full`}
            >
              {profitLoss > 0 ? "+" : ""}
