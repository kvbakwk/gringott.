...
export default function DashboardNav() {
  ...
  return (
    <div className="flex flex-col items-center gap-[10px] text-primary w-full mt-[30px] px-[30px] py-[10px]">
      ...
      <DashboardNavSubitem icon="payments" text="spłaty" path={`/${RouteSegments.Transactions}/${RouteSegments.Lending}`} show={pathname.startsWith(`/${RouteSegments.Transactions}`)} />
...
function DashboardNavItem({ icon, text, path }) {
  ...
  return (
    <Link
      href={path}
      className={clsx(
        "flex items-center gap-[18px] font-extralight text-[20px] w-full h-[50px] p-[18px] rounded-2xl cursor-pointer hover:bg-surface",
        pathname === path && "bg-surface shadow-sm"
      )}
    >
...
function DashboardNavSubitem({ icon, text, path, show }) {
  ...
  return (
    <Link
      href={path}
      className={clsx(
        "items-center gap-[18px] font-extralight text-[18px] w-[80%] h-[40px] p-[18px] rounded-2xl cursor-pointer hover:bg-surface",
        pathname === path && "bg-surface shadow-sm",
        show ? "flex" : "hidden"
      )}
    >
