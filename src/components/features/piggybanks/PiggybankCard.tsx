...
import { parseMoney, parseDate } from "@app/utils/parser";
...
export default function PiggybankCard({
  ...
  const iconName = useMemo(() => {
    ...
    if (n.includes("wakacje") || n.includes("trip") || n.includes("grecja")) return "flight";
    if (n.includes("auto") || n.includes("samochód")) return "directions_car";
    if (n.includes("dom") || n.includes("mieszkanie") || n.includes("remont")) return "home";
    if (n.includes("macbook") || n.includes("komputer") || n.includes("laptop")) return "laptop_mac";
  ...
  return (
    <div className={`flex flex-row items-center justify-between rounded-2xl p-5 transition-all group relative ${getBorderClass()} ${getShadowClass()} ${getBackgroundClass()}`}>
      {/* Left side: Icon + Name */}
      <div className="flex items-center gap-4 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="text-xs text-on-surface-variant mt-0.5">Skarbonka</div>
        </div>
      </div>
