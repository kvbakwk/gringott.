...
export default function GoalCard({
  ...
  const progress =
    targetAmount > 0 ? Math.min((wallet.balance / targetAmount) * 100, 100) : 0;
  ...
  return (
    <div
      className={`flex flex-col gap-4 rounded-2xl p-5 transition-all group relative ${getBorderClass()} ${getShadowClass()} ${getBackgroundClass()}`}
    >
      {/* Top row: Icon + Name + Menu */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4 min-w-0">
          <div
            className={`w-12 min-w-[48px] h-12 rounded-full flex items-center justify-center ${isComplete ? "bg-green-100 text-green-600" : "bg-primary/10 text-primary"}`}
          >
            <Icon className="text-2xl">
              {isComplete ? "check_circle" : iconName}
            </Icon>
          </div>
      ...
      <div className="flex items-baseline justify-between">
        ...
        <div
          className={`text-sm font-bold ${isComplete ? "text-green-600" : "text-on-surface-variant"}`}
        >
          {progress.toFixed(0)}%
        </div>
      </div>
